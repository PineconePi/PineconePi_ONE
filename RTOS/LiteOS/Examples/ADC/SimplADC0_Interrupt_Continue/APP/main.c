...
int main(void)
{
	...
	ADC_initStruct.HFULL_IEn = ADC_CH3;
...
void ADC0_Handler(void)
{
	...
	ADC0->IF = (1 << ADC_IF_CH3HFULL_Pos);
	
	while((ADC0->CH[3].STAT & ADC_STAT_EMPTY_Msk) == 0)
		printf("%4d\t%4d\t%4d\t%4d\r\n", ADC_Read(ADC0, ADC_CH0), ADC_Read(ADC0, ADC_CH1), ADC_Read(ADC0, ADC_CH2), ADC_Read(ADC0, ADC_CH3));
	
	temp = ADC_Read(ADC0, ADC_CH0), ADC_Read(ADC0, ADC_CH1), ADC_Read(ADC0, ADC_CH2), ADC_Read(ADC0, ADC_CH3); temp = temp;	//清除FIFO中残留数据
