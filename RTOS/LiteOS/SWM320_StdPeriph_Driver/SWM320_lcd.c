...
* 注意事项: 无
******************************************************************************************************************************************/
void SWM320_LCD_Init(LCD_TypeDef * LCDx, LCD_InitStructure * initStruct)
{	
	switch((uint32_t)LCDx)
	{
	case ((uint32_t)SWM320_LCD):
	...
	LCDx->IE = 1;
	LCDx->IF = 1;	//清除标志
	if(initStruct->IntEOTEn) SWM320_LCD_INTEn(LCDx);
	else					 SWM320_LCD_INTDis(LCDx);
...
* 注意事项: 无
******************************************************************************************************************************************/
void SWM320_LCD_Start(LCD_TypeDef * LCDx)
{
...
* 注意事项: 无
******************************************************************************************************************************************/
uint32_t SWM320_LCD_IsBusy(LCD_TypeDef * LCDx)
{
...
* 注意事项: 无
******************************************************************************************************************************************/
void SWM320_LCD_I80_WriteReg(LCD_TypeDef * LCDx, uint16_t reg, uint16_t val[], uint16_t cnt)
{
	SWM320_LCD->SRCADDR = (uint32_t)val;
	SWM320_LCD->CR0 &= ~LCD_CR0_DLEN_Msk;
	SWM320_LCD->CR0 |= ((cnt-1) << LCD_CR0_DLEN_Pos);
	
	SWM320_LCD->CR1 |=  (1 << LCD_CR1_CMD_Pos);
	SWM320_LCD->CR1 &= ~LCD_CR1_REG_Msk;
	SWM320_LCD->CR1 |= (reg << LCD_CR1_REG_Pos);
	
	SWM320_LCD_Start(LCDx);
	while(SWM320_LCD_IsBusy(LCDx));
}
...
* 注意事项: 无
******************************************************************************************************************************************/
void SWM320_LCD_I80_WriteOneReg(LCD_TypeDef * LCDx, uint16_t reg, uint16_t val)
{
	uint16_t buf[1] __attribute__((aligned(4)));
		
	buf[0] = val;

	SWM320_LCD_I80_WriteReg(LCDx, reg, buf, 1);
}
...
* 注意事项: 无
******************************************************************************************************************************************/
void SWM320_LCD_I80_WriteData(LCD_TypeDef * LCDx, uint16_t val[], uint16_t cnt)
{
	SWM320_LCD->SRCADDR = (uint32_t)val;
	SWM320_LCD->CR0 &= ~LCD_CR0_DLEN_Msk;
	SWM320_LCD->CR0 |= ((cnt-1) << LCD_CR0_DLEN_Pos);
...
* 注意事项: 无
******************************************************************************************************************************************/
void SWM320_LCD_I80_WriteOneData(LCD_TypeDef * LCDx, uint16_t val)
{
	uint16_t buf[1] __attribute__((aligned(4)));
	
	buf[0] = val;

	SWM320_LCD_I80_WriteData(LCDx, buf, 2);
}
...
* 注意事项: 无
******************************************************************************************************************************************/
void SWM320_LCD_INTEn(LCD_TypeDef * LCDx)
{
	LCDx->IM = 0;
}
...
* 注意事项: 无
******************************************************************************************************************************************/
void SWM320_LCD_INTDis(LCD_TypeDef * LCDx)
{
	LCDx->IM = 1;
}
...
* 注意事项: 无
******************************************************************************************************************************************/
void SWM320_LCD_INTClr(LCD_TypeDef * LCDx)
{
	LCDx->IF = 1;
}
...
* 注意事项: 无
******************************************************************************************************************************************/
uint32_t SWM320_LCD_INTStat(LCD_TypeDef * LCDx)
{
