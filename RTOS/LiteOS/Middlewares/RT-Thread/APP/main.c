...
void TaskADC(void *arg)
{
	...
	PWM_initStruct.mode = PWM_MODE_COMPL_CALIGN;//A路和B路互补中心对称输出				
	PWM_initStruct.cycleA = 50000;				//2.5M/50000 = 50Hz			
	PWM_initStruct.hdutyA = 25000;				//2500/10000 = 50%
...
{	
	ADC0->IF = (1 << ADC_IF_CH0EOC_Pos);	//清除中断标志
	
	rt_mb_send(&queueADC, ADC_Read(ADC0, ADC_CH0));
}
...
void TaskPWM(void *arg)
{
	...
	while(1)
	{
		...
		{
			PWM_SetHDuty(PWM1, PWM_CH_A, 10000 * duty / 4096);
			PWM_SetHDuty(PWM1, PWM_CH_B, 10000 - PWM_GetHDuty(PWM0, PWM_CH_A));
		}
	}
}
